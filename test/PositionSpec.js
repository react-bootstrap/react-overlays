...
describe('<Position>', () => {
  ...
  it('should accept a function child', () => {
    let instance = ReactTestUtils.renderIntoDocument(
      <Position>{props => <Span {...props} />}</Position>
    )
      ...
      it('should calculate the correct position', async () => {
        ...
        ;[instance.overlay1, instance.overlay2].forEach(({ props }) => {
          ...
          expect(props.style.transform).to.eql(
            `translate3d(${expected.position.left}px, ${
              expected.position.top
            }px, 0)`
          )
    ...
    ;[
      ...
      describe(`placement = ${placement}`, () => {
        describe('no viewport offset', () => {
          checkPosition(placement, { left: 250, top: 250 }, testCase.noOffset)
        })
        ...
        render() {
          return (
            <div>
              ...
              <Position target={() => this.target} shouldUpdatePosition>
                <Span fakeProp={this.state.fakeProp} />
              </Position>
            </div>
          )
  ...
  it('should not forward own props to child', () => {
    ...
    ReactTestUtils.renderIntoDocument(
      <Position target={() => null} otherProp="foo" className="bar">
        <Child className="foo" />
      </Position>
    )
